...
# run code ->uvicorn user_page:app --reload
...
def delete_ticket(booking_id:int):
    ...
    if not booking:
        cur6.close()
        conn.close()
        return {"message":"Booking_id doenot exist"}

    cur6.execute("delete from flight_book_details where booking_id = %s;",(booking_id,))

    conn.commit()
    cur6.close()
    conn.close()
    return {"message": "Booking cancelled successfully! Do visit Again !!","cancelled_booking_id ":{booking_id}}
...
def get_user_info(user_id: int):
    ...
    cur9 = conn.cursor()
    try:
        cur9.execute("SELECT * FROM user_logs WHERE user_id = %s", (user_id,))
        user_info = cur9.fetchone()
        if user_info:
...
# @app.post("/admin/add_new_flight")
# def add_new_flight(flight_data: dict):
#     conn = get_db_connection()
#     cur = conn.cursor()
#     try:
#         cur.execute("INSERT INTO flights (flight_name, departure_time, arrival_time) VALUES (%s, %s, %s)",
#                     (flight_data["flight_name"], flight_data["departure_time"], flight_data["arrival_time"]))
#         conn.commit()
#         return {"response": "Flight added successfully!"}
#     finally:
#         cur.close()
#         conn.close()
